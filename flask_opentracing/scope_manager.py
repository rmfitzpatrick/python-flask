...
from opentracing.scope_managers import ThreadLocalScopeManager, constants
...
class FlaskScopeManager(ThreadLocalScopeManager):
    ...
    def activate(self, span, finish_on_close):
        """
        Make a opentracing.Span instance active. Returns an opentracing.Scope instance
        to control the end of the active period for the opentracing.Span. It is a
        programming error to neglect to call Scope.close() on the returned instance.
        ...
        setattr(self._context, constants.ACTIVE_ATTR, scope)
        return scope

    @property
    def active(self):
        """
        Returns the currently active opentracing.Scope which can be used to access
        the currently active Scope.span or None if not available.
        """
        return getattr(self._context, constants.ACTIVE_ATTR, None)
...
class _FlaskScope(Scope):
    ...
    def close(self):
        ...
        setattr(self.manager._context, constants.ACTIVE_ATTR, self._to_restore)
